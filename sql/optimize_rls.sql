...
-- Drop potentially existing new policies (for re-runs)
DROP POLICY IF EXISTS "trading_sessions_select" ON trading_sessions_v2;
DROP POLICY IF EXISTS "trading_sessions_admin_insert" ON trading_sessions_v2;
DROP POLICY IF EXISTS "trading_sessions_admin_update" ON trading_sessions_v2;
DROP POLICY IF EXISTS "trading_sessions_admin_delete" ON trading_sessions_v2;

CREATE POLICY "trading_sessions_select" ON trading_sessions_v2 FOR SELECT TO authenticated
USING (status IN ('pending', 'running') OR EXISTS (SELECT 1 FROM user_profiles WHERE id = (select auth.uid()) AND is_admin = true));
...
DROP POLICY IF EXISTS "session_participants_access" ON session_participants;

CREATE POLICY "session_participants_access" ON session_participants FOR ALL TO authenticated
USING (user_id = (select auth.uid()) OR EXISTS (SELECT 1 FROM user_profiles WHERE id = (select auth.uid()) AND is_admin = true));


-- 3. USER TRADING SETTINGS
DROP POLICY IF EXISTS "user_settings_own" ON user_trading_settings;
CREATE POLICY "user_settings_own" ON user_trading_settings FOR ALL TO authenticated USING (user_id = (select auth.uid()));


-- 4. SYSTEM NOTIFICATIONS
DROP POLICY IF EXISTS "notifications_user" ON system_notifications;
DROP POLICY IF EXISTS "notifications_admin_insert" ON system_notifications;
DROP POLICY IF EXISTS "notifications_admin_update" ON system_notifications;
DROP POLICY IF EXISTS "notifications_admin_delete" ON system_notifications;

CREATE POLICY "notifications_user" ON system_notifications FOR SELECT TO authenticated USING (user_id = (select auth.uid()) OR user_id IS NULL);
...
DROP POLICY IF EXISTS "trade_logs_select" ON trade_logs;
DROP POLICY IF EXISTS "trade_logs_admin_insert" ON trade_logs;
DROP POLICY IF EXISTS "trade_logs_admin_update" ON trade_logs;
DROP POLICY IF EXISTS "trade_logs_admin_delete" ON trade_logs;

CREATE POLICY "trade_logs_select" ON trade_logs FOR SELECT TO authenticated
USING (user_id = (select auth.uid()) OR EXISTS (SELECT 1 FROM user_profiles WHERE id = (select auth.uid()) AND is_admin = true));
...
CREATE POLICY "activity_logs_admin" ON activity_logs_v2 FOR ALL TO authenticated USING (EXISTS (SELECT 1 FROM user_profiles WHERE id = (select auth.uid()) AND is_admin = true));
